# -*- coding: utf-8 -*-
"""
Created on Wed May 22 11:55:42 2019
...
list_file = os.listdir(path+'/baby_names')
df = pd.DataFrame()
...
for file in list_file:
     if file=='yob2011.txt':
         ...
         if file.endswith(".txt"):
             fp = open(file,"rt")
             year = file[3:file.find(".")]
             data = pd.Series(fp.readlines())
             new_df = pd.DataFrame(data,columns=[year])
...
path = os.getcwd()
os.chdir(path+"\\baby_names")
1#list_file = os.listdir(path+'/baby_names')
...
print("Top 5 male baby names in 2017\n",df1[(df1["Sex"] == "M") & (df1["Year"] == 2017)].head(5))
